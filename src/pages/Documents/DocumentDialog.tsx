...
const DocumentDialog = ({ isOpen, onClose, onDocumentCreated, document }: DocumentDialogProps) => {
  ...
  // Watch for currency changes and update transactions
  const selectedCurrency = form.watch('currency');
  
  useEffect(() => {
    // Update all transactions when currency changes
    if (selectedCurrency) {
      setTransactions(prev => prev.map(t => ({ ...t, currency: selectedCurrency })));
      setParallelTransactions(prev => prev.map(t => ({ ...t, currency: selectedCurrency })));
    }
  }, [selectedCurrency]);
      ...
      const allTransactionsSafe = allTransactions.map((t, idx) => ({
        ...
        description:
          typeof t.description === "string" && t.description.trim() !== ""
            ? t.description
            : "",
        currency: data.currency, // Use the document's currency
      }));
  ...
  const addTransaction = async (transaction: Transaction) => {
    const transactionWithCurrency = { ...transaction, currency: selectedCurrency };
    ...
    <>
      <Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Waluta dokumentu</FormLabel>
                      <FormControl>
                        <CurrencySelector
                          value={field.value}
                          onChange={field.onChange}
                      ...
                      {!locationSettings?.allow_foreign_currencies && (
                        <p className="text-xs text-gray-500">
                          Ta placówka nie ma uprawnień do operacji walutami obcymi
                        </p>
                      )}
                    </FormItem>
                  )}
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setShowParallelSection(!showParallelSection)}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                      <div className="flex items-center gap-2 text-yellow-800">
                        ...
                        <span className="text-sm font-medium">
                          Operacje główne nie są zbilansowane. Różnica: {Math.abs(debitTotal - creditTotal).toLocaleString('pl-PL', { style: 'currency', currency: selectedCurrency || 'PLN' })}
                        </span>
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-4 border-t pt-6 bg-gray-50 p-4 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <Button
                  ...
                  className="flex items-center gap-2"
                  disabled={isEditingBlocked || showParallelInlineForm}
                >
                  <Plus className="h-4 w-4" />
              ...
              <div className="space-y-4">
                    ...
                    <TableBody>
                      {parallelTransactions.map((transaction, index) => (
                        <InlineEditTransactionRow
                          ...
                          transaction={transaction}
                          index={index}
                          isEditing={editingParallelTransactionIndex === index}
                          onEdit={() => handleEditParallelTransaction(index)}
                          onSave={(updatedTransaction) => handleEditParallelTransactionWithBalancing(index, updatedTransaction)}
                          onCancel={handleCancelParallelEdit}
                ...
                {parallelTransactions.length > 0 && (
                  <div className="border-t pt-4 space-y-2">
                    ...
                    {Math.abs(parallelDebitTotal - parallelCreditTotal) > 0.01 && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                        <div className="flex items-center gap-2 text-yellow-800">
                          <AlertTriangle className="h-4 w-4" />
                          <span className="text-sm font-medium">
                            Księgowanie równoległe nie jest zbilansowane. Różnica: {Math.abs(parallelDebitTotal - parallelCreditTotal).toLocaleString('pl-PL', { style: 'currency', currency: selectedCurrency || 'PLN' })}
                          </span>
                        </div>
                      </div>
                    )}
          ...
          {(transactions.length > 0 || parallelTransactions.length > 0) && (
            <div className="border-t pt-4 space-y-2 bg-blue-50 p-4 rounded-lg">
              ...
              <div className="flex justify-between items-center text-xl font-bold">
                ...
                <span className={cn(
                  ...
                  {Math.abs((debitTotal + parallelDebitTotal) - (creditTotal + parallelCreditTotal)) <= 0.01 
                    ? "ZBILANSOWANE" 
                    : `RÓŻNICA: ${Math.abs((debitTotal + parallelDebitTotal) - (creditTotal + parallelCreditTotal)).toLocaleString('pl-PL', { style: 'currency', currency: selectedCurrency || 'PLN' })}
                  `}
                </span>
              </div>
            </div>
...
const InlineEditTransactionRow: React.FC<InlineEditTransactionRowProps> = ({
  ...
  return (
    <TableRow className={cn(
      "border-2",
      isParallel ? "bg-purple-50 border-purple-200" : "bg-blue-50 border-blue-200"
    )}>
      <TableCell>
        <Textarea
        ...
        <div className="flex gap-2">
          {!isParallel && onParallelPosting && showCopyButton && (
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={onParallelPosting}
              disabled={isEditingBlocked}
              title="Kopiuj do księgowania równoległego"
            >
              <Copy className="h-4 w-4" />
            </Button>
          )}

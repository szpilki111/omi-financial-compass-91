...
const ExcelFormImportDialog: React.FC<ExcelFormImportDialogProps> = ({ open, onClose, onImportComplete }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setParseError(
        "Nie udało się przetworzyć pliku Excel. Upewnij się, że używasz właściwego szablonu formularza rozliczeń.",
      );
    }
  };
  ...
  const downloadTemplate = async () => {
    try {
      ...
      const { data, error } = await supabase.storage.from("Document").download("1.xlsx");
        ...
        <div className="flex-1 overflow-y-auto space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              <input type="file" id="excel-file" accept=".xls,.xlsx" onChange={handleFileChange} className="hidden" />
              <Button
                type="button"
                variant="outline"
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-muted-foreground">
                  Łączna kwota:{" "}
                  {new Intl.NumberFormat("pl-PL", { style: "currency", currency: "PLN" }).format(totalAmount)}
                </span>
              </div>

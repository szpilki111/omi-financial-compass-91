-- Funkcja do pobierania kont użytkownika z filtrowaniem server-side
...
CREATE OR REPLACE FUNCTION public.get_user_filtered_accounts(
  p_user_id uuid,
...
BEGIN
  ...
  -- Admin i prowincjał z skip_restrictions widzą wszystkie konta
  IF (user_role = 'admin' OR user_role = 'prowincjal') AND p_skip_restrictions THEN
    RETURN QUERY
  ...
  WHERE (p_include_inactive OR a.is_active = true)
    AND (
      ...
      OR
      -- Pasuje do identyfikatora lokalizacji (2 liczby po pierwszym myślniku)
      EXISTS (
        SELECT 1 FROM unnest(location_identifiers) AS loc_id
        WHERE 
          -- Sprawdź czy parts[1] i parts[2] numeru konta = identifier
          split_part(a.number, '-', 2) = split_part(loc_id, '-', 1)

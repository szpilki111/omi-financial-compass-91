...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!profile) {
      ...
      return new Response(JSON.stringify({ success: true, message: "Jeśli konto istnieje, email został wysłany" }), {
        status: 200,
    ...
    const appUrl = "https://omi-repo.netlify.app/";
    ...
    return new Response(JSON.stringify({ success: true, message: "Email z linkiem do resetu hasła został wysłany" }), {
      status: 200,
      headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  } catch (error: any) {
...
function buildPasswordResetEmail(userName: string, resetUrl: string): string {
  ...
  html +=
    '<p style="margin:0;color:#64748b;font-size:14px;">Jeśli to nie Ty prosiłeś o reset hasła, zignoruj tę wiadomość. Twoje konto jest bezpieczne.</p></div>';

  html += "</td></tr>";

  // Footer
  html +=
    '<tr><td style="padding:24px 40px;background-color:#f8fafc;border-radius:0 0 12px 12px;border-top:1px solid #e2e8f0;">';
  html +=
    '<p style="margin:0;color:#64748b;font-size:13px;text-align:center;">Ta wiadomość została wygenerowana automatycznie przez System Finansowy OMI.</p>';
